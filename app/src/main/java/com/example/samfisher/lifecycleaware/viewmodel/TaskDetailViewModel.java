...
public class TaskDetailViewModel extends ViewModel {
    ...
    Disposable disposable = Observable.just(s)
        .switchMap(new Function<CharSequence, ObservableSource<?>>() {
          ...
          public ObservableSource<?> apply(CharSequence charSequence) throws Exception {
            return Observable
                .just(charSequence).delay(2, TimeUnit.SECONDS)
        ...
        .subscribe(
            taskEntity -> {
              Log.d(TAG, "searchTask: " + taskEntity);
              searchMediatorLiveData.setValue(Resource.success(taskEntity));
            },
            throwable -> error.setValue(Resource.error(throwable.getMessage(), throwable))
        );
    Log.d(TAG, "loadSearchTask: " + disposable);
        /*.doOnNext(taskEntity -> Log.d(TAG, "accept: " + taskEntity))
        .switchMap(charSequence -> {
          //taskInteractor.search(charSequence.toString())
          List<TaskEntity> taskEntities = new ArrayList<>();
          TaskEntity taskEntity = new TaskEntity();
          taskEntities.add(taskEntity);
          return Observable.just(taskEntities).delay(5, TimeUnit.SECONDS);
        })
        .observeOn(AndroidSchedulers.mainThread())
  ...
  public MediatorLiveData<Resource> searchTask() {
    ...
    searchMediatorLiveData.addSource(charSequenceMutableLiveData, this::loadSearchTask);
    searchMediatorLiveData
        .addSource(rxSearchLiveData, listResource -> Log.d(TAG, "onChanged: " + listResource));
